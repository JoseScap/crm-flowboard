...
-- View: user_api_keys_view
-- This view provides a secure way to query user_api_keys by obfuscating the 'key' column
-- The 'key' column is replaced with a dummy value to prevent accidental exposure
-- Even if someone tries to SELECT the 'key' column, they will only get the obfuscated value
DROP VIEW IF EXISTS user_api_keys_view CASCADE;
CREATE VIEW user_api_keys_view AS
SELECT 
  id,
  user_id,
  '••••••••••••••••••••••••••••••••••••••••'::TEXT AS key, -- Dummy obfuscated value (always the same)
  key_index,
  is_active,
  created_at,
  last_rotated_at
...
-- This ensures the view executes with the permissions of the user invoking it
-- rather than the view owner, allowing RLS policies to work correctly
ALTER VIEW user_api_keys_view SET (security_invoker = true);
...
CREATE TABLE pipeline_stage_leads (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
-- Enable Row Level Security on pipeline_stage_leads table
ALTER TABLE pipeline_stage_leads ENABLE ROW LEVEL SECURITY;

-- Pipeline stage leads indexes
DROP INDEX IF EXISTS idx_pipeline_stage_leads_pipeline_stage_id;
DROP INDEX IF EXISTS idx_pipeline_stage_leads_created_at;
DROP INDEX IF EXISTS idx_pipeline_stage_leads_business_id_created_at;
CREATE INDEX idx_pipeline_stage_leads_pipeline_stage_id ON pipeline_stage_leads(pipeline_stage_id);
CREATE INDEX idx_pipeline_stage_leads_created_at ON pipeline_stage_leads(created_at);
CREATE INDEX idx_pipeline_stage_leads_business_id_created_at ON pipeline_stage_leads(business_id, created_at);
...
CREATE TABLE sales (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    business_id BIGINT NOT NULL REFERENCES businesses(id) ON DELETE RESTRICT,
    lead_id BIGINT REFERENCES pipeline_stage_leads(id) ON DELETE RESTRICT,
